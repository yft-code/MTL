<template>
    <div class="start-test">
        ...
        <edit-test-content ref="editTestContentRef" :table-name="axiosList.table_name" :custom-cols="customCols" @afterEdit="afterEditTestContent"/>
        <add-test-mission ref="addTestMissionRef" :currentQa="qaExcutor" :androidNum="testDetail.android_num" :iosNum="testDetail.ios_num" @afterEdit="getTestData" @afterAllocate="getCurrentApply"></add-test-mission>
        ...
        <device-select ref="deviceSelectRef" @afterEdit="afterEditTestContent"></device-select>
        <add-mission ref="addMissionRef" :table-name="axiosList.table_name" @afterEdit="afterEditTestContent"/>
        <div class="container">
            ...
            <div v-if="canLoadTable">
                <div style="margin-bottom:1rem;">
                    ...
                    <!-- <el-button v-if="currentQaAdmin.indexOf(userEmail) !== -1" type="primary" size="small" @click="addTestContent">新增测试任务</el-button> -->
                    <!-- <el-button v-if="currentQaAdmin.indexOf(userEmail) !== -1 || isAdmin" type="primary" size="small" @click="addTestMission">分配测试任务</el-button> -->
                    <el-button v-if="(currentQaAdmin.indexOf(userEmail) !== -1 || isAdmin) && qaExcutor == ''" type="primary" size="small" @click="addTestMission">分配测试任务</el-button>
                    <el-button v-if="(currentQaAdmin.indexOf(userEmail) !== -1 || isAdmin) && qaExcutor != ''" type="primary" size="small" @click="addTestMission">重新分配测试任务</el-button>
                ...
                <!-- 测试要求 -->
                <test-requirement :total="total" :total_finished="total_finished" :testDetail="testDetail" :testRequest="testRequest" :simulatorData="simulatorData" :serverName="'compatibility'"></test-requirement>
                
                <div v-for="item,k in testData" class="box-card" :key="k+'_'+item.test_qa">
                    <div class="report-card">
                        <!-- <div class="card-title" @click="changeExpand(item,k)" > -->
                        <div class="card-title">
                            <div class="title">{{ item.test_qa_name || item.test_qa.split('@')[0] }}</div>
                            <div class="other-span">
                                已完成 {{item.completed}} 条 / 总共 {{item.total}} 条
                                <!-- <el-button type="" size="mini" @click="addMission(item.test_qa)" v-if="currentQaAdmin.indexOf(userEmail) !== -1 || (item.test_qa == userEmail && qaExcutor.indexOf(userEmail) !== -1)" style="margin-left:10px;">新增任务</el-button> -->
                                <el-button type="" size="mini" @click="addMission(item.test_qa)" v-if="(item.test_qa == userEmail && qaExcutor.indexOf(userEmail) !== -1)" style="margin-left:10px;">新增任务</el-button>
                            </div>
...
<script>
import EditColumn from "@/components/StartTest/Compatibility/EditColumn"
import EditTestContent from "@/components/StartTest/Compatibility/EditTestContent"
import TestCover from "@/components/StartTest/Compatibility/TestCover"
import EditTestCover from "@/components/StartTest/Compatibility/EditTestCover"
import AddTestMission from '@/components/StartTest/Compatibility/AddTestMission'
import RecordTable from '@/components/StartTest/Common/RecordTable'
import EditMission from '@/components/StartTest/Compatibility/EditMission'
...
import EditPatchMission from '@/components/StartTest/Compatibility/EditPatchMission'
import DeviceSelect from '@/components/StartTest/Compatibility/DeviceSelect'
import AddMission from '@/components/StartTest/Compatibility/AddMission'
export default {
    components: {
        EditColumn,
        EditTestContent,
    ...
    data() {
        return {
            ...
            testRequest: {
                Android: [
                    {
                        propName:"android_num",
                        titleName:"Android测试数量",
                    },
                    {
                        propName:"android_os",
                        titleName:"Android最低操作系统",
                    },
                    {
                        propName:"android_ram",
                        titleName:"Android最低内存",
                    },
                    {
                        propName:"android_cpu",
                        titleName:"Android最低CPU",
                    },
                    {
                        propName:"android_other",
                        titleName:"Android其他",
                    },
                ],
                iOS: [
                ...
                PC: [
                    {
                        propName:"pc_cpu_model",
                        titleName:"处理器",
                    },
                    {
                        propName:"pc_gpu_model",
                        titleName:"显卡",
                    },
                    {
                        propName:"pc_other",
            ...
            tableList: {
                defaultRows: 10,
                postApp: "get_expert_test_record",
                postAppPrefix: "expertCompatibility",
                sortKey: "sort_field",
                tableDataKey: "rows",
                tableTotalKey: "total",
                needGetRecordQuestionlist: true,
                columnData: []
            },
            basicColumnData: [],
    ...
    methods: {
        ...
        querySearch(queryString, cb){
            ...
            var results = queryString ? this.searchList.filter(this.createFilter(queryString)) : this.searchList;
            cb(results);
            // return results
        },
        ...
        createFilter(queryString) {
            return (device) => {
                return (device.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0);
            };
        },
        ...
        getSerchKey(){
            ...
            this.$codePost("/expertCompatibility/associate_preset_device/", list, {operation: "获取预分配设备联想数据", failed: true }).then(res => {
                if(res.code == "200"){
                    // this.searchList = res.data
                    let searchList = []
                    res.data.forEach(device => {
                        let deviceItem = {}
                        deviceItem['value'] = device
                        searchList.push(deviceItem)
                    })
        ...
        deviceSelect(item){
            this.$refs.deviceSelectRef.deviceSelect(item);
        },
        ...
        getSimulatorData(){
            this.$codePost("/expertCompatibility/get_simulator_list/").then(res => {
                if(res.code == "200"){
                    this.simulatorData = res.data
                    // console.log(this.simulatorData)
                }
            })
        },
        // 获取测试记录
        getTestData(){
            ...
            this.$codePost("/expertCompatibility/get_expert_test_record/", list).then(res => {
                if(res.code == "200"){
                    // console.log(res)
                    this.testData = res.data.records
                    this.recordQuestionlist = res.data.question_data
                    this.total = res.data.total
                    this.total_finished = res.data.total_finished ? res.data.total_finished : 0
        ...
        addMission(testQa){
            ...
            this.$refs.addMissionRef.openDialog(testQa)
        },
        ...
        getTableAndCustomColumn() {
            ...
            // 这里需要获取自定义列接口
            this.$codePost("/expertCompatibility/get_custom_cols/", list, { operation: "获取自定义列", failed: true }).then(res => {
                if (res.code == 200) {
        ...
        deleteTestContent(item) {
            ...
            this.$selfConfirm(confirmText, '提示', { confirmButtonText: '确定', cancelButtonText: '取消' }).then(() => {
                ...
                this.$codePost("/expertCompatibility/delete_expert_test_record/", list, { operation: "删除测试内容", success: true, failed: true }).then(res => {
                    if (res.code == 200) {
        ...
        editCasesProblem() {
            ...
            // window.open(href);
            this.$router.push({
                path: '/mtl_test_platform/page/questionTable',
